...
%% 2.1.3.(a) Cholesky IRF Plotting (alternative plotting method)
figure(2);

set(gcf,'name','Cholesky IRF')
...
title('prod to \epsilon^{os}')

subplot(3,3,2)
plot(-IR(:,2,1)); hold on 
plot(zeros(VARopt.nsteps),'--k','LineWidth',0.5); hold on
xlim([1 VARopt.nsteps]);
title('rea to \epsilon^{os}')

subplot(3,3,3)
plot(-IR(:,3,1)); hold on 
plot(zeros(VARopt.nsteps),'--k','LineWidth',0.5); hold on
xlim([1 VARopt.nsteps]);
title('rpo to \epsilon^{os}')

subplot(3,3,4)
plot( IR(:,1,2)); hold on 
plot(zeros(VARopt.nsteps),'--k','LineWidth',0.5); hold on
xlim([1 VARopt.nsteps]);
title('prod to \epsilon^{ad}')

subplot(3,3,5)
plot(IR(:,2,2)); hold on 
plot(zeros(VARopt.nsteps),'--k','LineWidth',0.5); hold on
xlim([1 VARopt.nsteps]);
title( 'rea to \epsilon^{ad}')

subplot(3,3,6)
plot(IR(:,3,2)); hold on 
plot(zeros(VARopt.nsteps),'--k','LineWidth',0.5); hold on
xlim([1 VARopt.nsteps]);
title( 'rpo to \epsilon^{ad}')

subplot(3,3,7)
plot(IR(:,1,3)); hold on 
plot(zeros(VARopt.nsteps),'--k','LineWidth',0.5); hold on
xlim([1 VARopt.nsteps]);
title('prod to \epsilon^{osd}')

subplot(3,3,8)
plot(IR(:,2,3)); hold on 
plot(zeros(VARopt.nsteps),'--k','LineWidth',0.5); hold on
xlim([1 VARopt.nsteps]);
title( 'rea to \epsilon^{osd}')

subplot(3,3,9)
plot(IR(:,3,3)); hold on 
plot(zeros(VARopt.nsteps),'--k','LineWidth',0.5); hold on
xlim([1 VARopt.nsteps]);
title( 'rpo to \epsilon^{osd}')

filename = 'IRF Plot - Cholesky(supply shock sign adjusted).jpg';
...
VARopt.ndraws = 36000; % Number of rotations needed
VARopt.sr_hor = 1;
VARopt.pctg = 68; 
...
set(gcf,'name','Sign Restriction (Agnostic) IRF')
set(gcf,'NumberTitle','off')
...
filename = 'IRF Plot - Sign Restriction (Agnostic).jpg';
saveas(gcf, filename, 'jpeg')
...
set(gcf,'name','Sign Restriction (Elasticity-Bounded) IRF')
set(gcf,'NumberTitle','off')
subplot(3,1,1)
...
filename = 'IRF Plot - Sign Restriction (Elasticity-Bounded).jpg';
saveas(gcf, filename, 'jpeg')
